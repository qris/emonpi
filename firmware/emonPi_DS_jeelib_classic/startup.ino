...
void CT_Detect()
{
  ...
  //delay(5000); // Unlike on the emonTx V3 since the emonPi is not powered by AC-DC circuit there should not be need for delay to allow residual AC in reservoir capacitor to diminish.
  digitalWrite(LEDpin, LOW);

  pinMode(A0, OUTPUT);
  analogWrite(A0, 255);
  analogWrite(A0, 0);     // Pull input high-low-high-low to discharge any residual charge to avoid incorrectly detecting AC wave when not present
  pinMode(A0, INPUT);
